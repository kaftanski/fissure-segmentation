...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, offset, new_offset):
        ...
        device = xyz.device
        idx = torch.zeros(new_offset[b-1].item(), device=device, dtype=torch.int32)
        tmp = torch.full(size=(n,), fill_value=1e10, device=device, dtype=torch.float)
...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample, xyz, new_xyz, offset, new_offset):
        ...
        device = xyz.device
        idx = torch.zeros(m, nsample, device=device, dtype=torch.int32)
        dist2 = torch.zeros(m, nsample, device=device, dtype=torch.float)
...
class Grouping(Function):
    ...
    def forward(ctx, input, idx):
        ...
        output = torch.zeros(m, nsample, c, device=input.device, dtype=torch.float)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros(n, c, device=grad_output.device, dtype=torch.float)
...
class Subtraction(Function):
    ...
    def forward(ctx, input1, input2, idx):
        ...
        assert input1.device.type == 'cuda'

        n, c = input1.shape; nsample = idx.shape[-1]
        # output = torch.cuda.FloatTensor(n, nsample, c).zero_()
        output = torch.zeros(n, nsample, c, device=input1.device, dtype=torch.float)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input1 = torch.zeros(n, c, device=grad_output.device, dtype=torch.float)
        grad_input2 = torch.zeros(n, c, device=grad_output.device, dtype=torch.float)
...
class Aggregation(Function):
    ...
    def forward(ctx, input, position, weight, idx):
        ...
        output = torch.zeros(n, c, device=input.device, dtype=torch.float)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros(n, c, device=grad_output.device, dtype=torch.float)
        grad_position = torch.zeros(n, nsample, c, device=grad_output.device, dtype=torch.float)
        grad_weight = torch.zeros(n, nsample, w_c, device=grad_output.device, dtype=torch.float)
...
def interpolation(xyz, new_xyz, feat, offset, new_offset, k=3):
    ...
    new_feat = torch.zeros(new_xyz.shape[0], feat.shape[1], device=xyz.device, dtype=torch.float)
    for i in range(k):
...
class Interpolation(Function):
    ...
    def forward(ctx, xyz, new_xyz, input, offset, new_offset, k=3):
        ...
        # output = torch.cuda.FloatTensor(n, c).zero_()
        output = torch.zeros(n, c, device=xyz.device, dtype=torch.float)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.zeros(m, c, device=grad_output.device, dtype=torch.float)
