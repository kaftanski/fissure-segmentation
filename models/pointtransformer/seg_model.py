    ...
    def forward(self, pxo):
        ...
        if self.stride != 1:
            ...
            for i in range(1, o.shape[0]):
                ...
                n_o.append(count)
            n_o = torch.tensor(n_o, device=p.device, dtype=o.dtype)
...
class PointTransformerSeg(nn.Module):
    ...
    def forward(self, pxo):
        ...
        :param pxo: tuple containing coords (n, 3), features (n, c), and batch offsets (b). All points in batch are
            viewed into the first dimension, so the offsets are used to separate the different point clouds.
            E.g. shape_i = coords[offsets[i]:offsets[i+1]]
        :return: segmentation (n, classes)
        """
...
class PointTransformerCompatibility(PointSegmentationModelBase):
    def __init__(self, in_features, num_classes, **kwargs):
        super().__init__(in_features, num_classes)
        self.point_transformer = pointtransformer_seg_repro(c=in_features, k=num_classes)

    def forward(self, x):
        bs, n_feat, npts = x.shape

        # convert to the input PT expects
        x_batch_reshaped_for_PT = x.transpose(1, 2).reshape(-1, n_feat)
        coords = x_batch_reshaped_for_PT[:, :3].contiguous()
        feat = x_batch_reshaped_for_PT[:, 3:].contiguous()
        offsets = (torch.arange(bs, dtype=torch.int32, device=x.device) + 1) * npts  # note: the point clouds may contain different amounts of points (I think)!
